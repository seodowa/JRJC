  ...
  return (
    <div className="flex h-full flex-col gap-7">
      <div className="flex-none">
        <h1 className="mb-7 text-4xl leading-none font-normal tracking-[-0.03em] md:text-5xl">Site content</h1>
        
        {/* Section Navigation */}
        <div className="flex gap-7 overflow-x-auto border-b border-line custom-scrollbar">
          {sections.map((section) => (
            <button
              key={section.key}
              onClick={() => scrollToSection(section.key)}
              className="-mb-px border-b-2 border-transparent pb-3 text-[15px] whitespace-nowrap text-ink-2 transition-colors hover:border-ink hover:text-ink"
            >
              {section.label}
            </button>
          ))}
