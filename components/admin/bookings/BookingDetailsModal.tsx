  ...
  const getStatusButtons = (status: string) => {
    switch (status) {
      case 'Pending':
        return (
          <>
            <AsyncButton onClick={() => onApprove(booking.Booking_ID)} disabled={isProcessing} className="px-4 py-2 bg-[#A1E3F9] text-white rounded-md hover:bg-blue-400 disabled:opacity-50">Approve</AsyncButton>
            <AsyncButton onClick={() => onDecline(booking.Booking_ID)} disabled={isProcessing} className="px-4 py-2 shadow-sm text-red-500 rounded-md hover:bg-red-400 hover:text-white disabled:opacity-50">Decline</AsyncButton>
          </>
        );
      case 'Confirmed':
        return (
          <>
            <AsyncButton onClick={() => onStart(booking.Booking_ID)} disabled={isProcessing} className="px-4 py-2 bg-[#A1E3F9] text-white rounded-md hover:bg-blue-400 disabled:opacity-50">Start</AsyncButton>
            <AsyncButton onClick={() => onCancel(booking.Booking_ID)} disabled={isProcessing} className="px-4 py-2 border border-gray-400 text-red-500 rounded-md hover:bg-red-400 hover:text-white disabled:opacity-50">Cancel</AsyncButton>
          </>
        );
      case 'Ongoing':
        return (
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <div className="bg-white rounded-lg shadow-xl p-6 md:p-8 relative">
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end space-x-3">
            ...
            <AsyncButton onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Close</AsyncButton>
          </div>
        </div>
      </Modal>
